...
def filter_seqs(sequences: DNAIterator,
                min_length: int=1,
                max_length: int=None,
                max_ambiguous_fraction: float=1.0) -> pd.Series:
    max_length = max_length or np.inf
    result = {}
    for sequence in sequences:
        sequence_length = len(sequence)
        ambiguous_fraction = sequence.degenerates().sum() / sequence_length
        too_long = sequence_length > max_length
        too_short = sequence_length < min_length
        too_ambiguous = ambiguous_fraction > max_ambiguous_fraction
